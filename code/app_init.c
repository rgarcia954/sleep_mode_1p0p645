...
void DeviceInit(void)
{
    ...
    for (uint8_t i = 0; i < GPIO_PAD_COUNT; i++)
    {
        SYS_GPIO_CONFIG(i, (GPIO_MODE_DISABLE | GPIO_LPF_DISABLE |
                            GPIO_WEAK_PULL_DOWN  | GPIO_2X_DRIVE));
    }
...
#if CHARGE_PUMP_ENABLE == 1

    ACS->VDDCP_CTRL = VDDCP_CPCLK_4KHZ | VDDCP_PTRIM_4MA | VDDCP_READY | VDDCP_COMP_ENABLED;
#endif    /* if CHARGE_PUMP_ENABLE == 1 */
