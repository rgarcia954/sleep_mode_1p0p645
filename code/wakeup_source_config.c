...
void Wakeup_Source_Config(void)
{
	if ((WAKEUP_SRC_FLAG_BIT_SET << WAKEUP_SRC_RTC_ALARM) & WAKEUP_SRC_EN_MSK)
	{
		...
		RTC_ALARM_Init(RTC_ALARM_DURATION);
	}
 ...
 *              RTC_ALARM_COUNT_THRES: RTC counter threshold
 */
...
void RTC_ALARM_Init(uint32_t alarm_duration)
{
    ...
    /* Disable RTC, RTC ALARM event */
    Sys_RTC_Config(RTC_CNT_START_4294967295, (RTC_CLK_SRC | RTC_DISABLE | RTC_ALARM_DISABLE));

    /* Reset the RTC Timer (prescale and counter) */
    Sys_RTC_Config(RTC_CNT_START_4294967295, RTC_RESET);

    /* Set RTC clock source
     * Enable RTC, enable RTC ALARM event */
    Sys_RTC_Config(RTC_CNT_START_4294967295, (RTC_CLK_SRC | RTC_ENABLE | alarm_duration));
...
void RTC_ClockSource_Init(void)
{
    ...
    else if (RTC_CLK_SRC == RTC_CLK_SRC_GPIO0)
    {
        /* Configure GPIO0 as an input pin */
        SYS_GPIO_CONFIG(GPIO0, GPIO_MODE_INPUT);
    }
    else if (RTC_CLK_SRC == RTC_CLK_SRC_GPIO1)
    {
